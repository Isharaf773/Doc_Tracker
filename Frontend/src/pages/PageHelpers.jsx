...
export const statuses = {
  ...
  approved: { label: "Approved", bg: TEAL_LIGHT, color: TEAL_TEXT },
  rejected: { label: "Rejected", bg: RED_LIGHT, color: RED },
...
export function Panel({ children, style, ...rest }) {
  return (
    <div {...rest} style={{ background: "rgba(255,255,255,0.96)", border: "none", borderRadius: 18, overflow: "hidden", boxShadow: "0 18px 35px rgba(15, 23, 42, 0.08)", ...style }}>
      {children}
    </div>
  );
}
...
export function PulseCard({ label, value, color, icon }) {
  return (
    <div className="pulse-card">
      <div className="pulse-card-top">
        <div className="pulse-label">{label}</div>
        {icon && (
          <div className="pulse-icon" style={{ color }}>
            <i className={`ti ${icon}`} style={{ fontSize: 16 }} />
          </div>
        )}
      </div>
      <div className="pulse-value" style={{ color }}>{value}</div>
      <div className="pulse-bar">
        <div className="pulse-bar-inner" style={{ width: "42%", background: color }} />
      </div>
    </div>
  );
}
